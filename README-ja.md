...
- **インストール不要で起動**: `npx statuscraft` / `bunx statuscraft` でデザイナーが即座に開く（クローン不要・ビルド不要）
...
npm から直接デザイナーを起動できます（クローン不要・ビルド不要）:
...
ビジュアルデザイナーが `http://localhost:4567` で起動し、ブラウザで開きます。ステータスラインを設計し、「Claude Code に適用（Apply to Claude Code）」をクリック（またはスクリプトをコピー）して、画面の手順に従ってください。`Ctrl+C` でローカルサーバーを停止します。

オプション: `--port <n>` でポート指定、`--no-open` でブラウザ自動起動を抑止。
