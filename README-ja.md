...
- **フルカラーモデル**: グループごとに前景・背景の hex ピッカー、色深度セレクタ（Truecolor `38;2`/`48;2`・256色・基本16色）
- **キュレーション済みテーマ**: Catppuccin / Dracula / Nord / Tokyo Night / Gruvbox / Rosé Pine をワンクリック適用（バリアント対応）
- **Powerlineモード**: 背景塗りつぶしセグメント + arrow/chevron/round/slant/flame/lego セパレータ（色ハンドオフ付き）、フォント不要の TUI スタイルも
- **トークン別アイコン**: 絵文字（デフォルト・フォント不要）/ プレーンUnicode / Nerd Font / ASCII の各ティア
- **リッチなプログレスバー**: solid / 8分割 / shaded / dots / line / numeric スタイル、幅指定、single / threshold / gradient の色モード
- **テキストスタイル**: グループごとに bold / dim / italic / underline と背景バッジ
- **グレースフルデグラデーション**: 文字セットティア切替、`NO_COLOR` / `FORCE_COLOR` 対応のランタイム色フォールバックをスクリプトに同梱
